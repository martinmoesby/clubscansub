        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var mailMessage = $"Hej {Input.Firstname}<br />," +
                    ...
                    var passwordMessage = $"Hej {Input.Firstname}<br />," +
                        $"Tillykke. Så er din klubkonto hos Dykkerklubben Scansub blevet oprettet.<br/>" +
                        $"Dit kodeord er <h2>{randomPassword}</h2> og du kan nu logge på sitet og evt. tilknytte din facebook-konto. <br />" +
                        $"Husk at dit kodeord er personligt og må ikke overdrages til andre. <br />" +
                        $"Du kan skifte dit kodeord under 'Min Konto' -> 'Kodeord' <br />" +
                        $"<br />" +
                        $"Med venlig hilsen <br />Scansub DK Diver";

                    await _emailSender.SendEmailAsync(Input.Email, "Din brugerkonto er blevet oprettet", passwordMessage);
